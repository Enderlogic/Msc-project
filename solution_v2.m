...
if strcmp(option_data, 'fic')
    % set the hyperparameters of data set (not all of them will be used)
    hyp_data.magnitude = 10; %squared magnitude
    hyp_data.lengthScale = 5;
    hyp_data.period = 100;
    hyp_data.sigma = 0.1;
...
prior.lengthScale = prior_gaussian('s2', 1, 'mu', 5);
...
num_rep = 400; % set the amount of samples

sample_hyp = sample_parameter(num_rep, x_train, data_train, prior, model_cov);
disp('Parameter sampling complete!')
%% Generate replicated data
% data_rep = sample_data(num_rep, sample_hyp, model_cov, x_train, data_train, x);
% data_rep = sample_data(num_rep, sample_hyp, model_cov, x);
data_test_rep = sample_data(num_rep, sample_hyp, model_cov, x_test);
disp('Data sampling complete!')
%% Compute posterior p-value
criteria = 'number_of_zero';
p_value = p_value_calculator(data_test, data_test_rep, criteria);
